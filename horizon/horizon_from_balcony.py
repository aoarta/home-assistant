...
#    "entity_id": "sensor.pv_production_power",          # Both together
    ...
    "pre_window": 3,  #6,          # x5 min that must be LOW before a rising edge
    "post_window": 3, #6,         # x5 min that must be HIGH after it
    "gap": 1, #2,                 # transition zone (15 min) between the windows
    "low_frac": 0.10, #0.20,         # LOW  = below 20 % of the day's reference max
    "high_frac": 0.15, #0.35,        # HIGH = above 35 % of the day's reference max
    "min_day_max": 500.0,     # skip days with day max below this (sensor units)
    ...
    # --- output files ---
    "out_points_csv": "horizon_points_balcony.csv",
    "out_profile_csv": "horizon_profile_balcony.csv",
    "out_plot_png": "horizon_plot_balcony.png",   # set to None to skip the plot
}
