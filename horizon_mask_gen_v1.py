...
For every calendar day (keyed "MM-DD", leap-safe) and every local hour the
script computes which fraction of that hour the sun is above the *local*
horizon (terrain), sampled in 5-min substeps. Hours with the sun behind a
ridge but above the astronomical horizon are reduced to the diffuse
fraction:
...
CONFIG = {
    ...
    "ref_year": 2024,             # leap year -> table includes Feb 29
}
# ---------------------------------------------------------------------------
...
def main():
    ...
    table = {}
    day = dt.date(cfg["ref_year"], 1, 1)
    one_day = dt.timedelta(days=1)
    n_sub = 60 // cfg["substep_min"]

    while day.year == cfg["ref_year"]:
    ...
    print(f"Built mask for {len(table)} days, {n_affected} affected hours")
